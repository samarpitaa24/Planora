...
class PomodoroTimer {
  ...
  initElements() {
    ...
    this.globalModal = document.getElementById('globalPomodoroModal');
    this.globalModalTitle = document.getElementById('globalModalTitle');
    this.globalModalMessage = document.getElementById('globalModalMessage');
    this.globalModalBtn1 = document.getElementById('globalModalBtn1');
    this.globalModalBtn2 = document.getElementById('globalModalBtn2');
  ...
  attachEventListeners() {
    // Global modal buttons (available on all pages) - attach only once
    this.attachGlobalModalListeners();

    // Only attach timer page controls when those elements exist
  ...
  attachGlobalModalListeners() {
    ...
    if (this.globalModalBtn1) {
      this.globalModalBtn1.addEventListener('click', (e) => {
        e.preventDefault();
        e.stopPropagation();
        this.handleGlobalModalBtn1();
      });
    }
    if (this.globalModalBtn2) {
      this.globalModalBtn2.addEventListener('click', (e) => {
        e.preventDefault();
        e.stopPropagation();
        this.handleGlobalModalBtn2();
      });
    }
  ...
  showModalCrossRoute(modalType) {
    // Play alarm immediately regardless of current page
    this.playAlarm();
  ...
  checkAndShowPendingModal() {
    ...
    // Show global modal if it exists (available on all pages)
    if (this.globalModal) {
      if (pendingModal === 'cycle') {
        this.showGlobalModal('Focus Cycle Completed!', 'Great job! Time for a break.', 'Start Break', 'Skip Break', 'cycle');
      } else if (pendingModal === 'break') {
        this.showGlobalModal('Break Time Over!', 'Ready to continue? Click below to start your next focus cycle.', 'Continue Studying', '', 'break');
      } else if (pendingModal === 'complete') {
        const totalMinutes = Math.floor(this.totalStudyTime / 60);
        let timeText = `${totalMinutes} minute${totalMinutes !== 1 ? 's' : ''}`;
        if (totalMinutes >= 60) {
          const hours = Math.floor(totalMinutes / 60);
          const mins = totalMinutes % 60;
          timeText = `${hours} hour${hours > 1 ? 's' : ''} ${mins} minute${mins !== 1 ? 's' : ''}`;
        }
        this.showGlobalModal('🎉 Session Completed!', `Congratulations! You've completed all cycles.\n\nTotal Study Time: ${timeText}\n\nGreat work! Keep up the momentum! 💪`, 'Awesome!', '', 'complete', true);
      }
    } else {
      // Fallback: Check if modal elements exist (user is on timer page)
  ...
  showGlobalModal(title, message, btn1Text, btn2Text, modalType, isCelebration = false) {
    if (!this.globalModal) return;
    
    // Store modal type for button handlers
    this.currentModalType = modalType;
    
    // Update modal content
    this.globalModalTitle.textContent = title;
    
    // Handle multiline messages
    this.globalModalMessage.textContent = message;
    
    // Update buttons
    this.globalModalBtn1.textContent = btn1Text;
    this.globalModalBtn1.style.display = btn1Text ? 'inline-block' : 'none';
    
    this.globalModalBtn2.textContent = btn2Text;
    this.globalModalBtn2.style.display = btn2Text ? 'inline-block' : 'none';
    
    // Add celebration styling if needed
    if (isCelebration) {
      this.globalModal.querySelector('.modal-content').classList.add('celebration');
    } else {
      this.globalModal.querySelector('.modal-content').classList.remove('celebration');
    }
    
    // Show modal
    this.globalModal.classList.add('show');
  }

  closeGlobalModal() {
    if (!this.globalModal) return;
    this.globalModal.classList.remove('show');
  }

  handleGlobalModalBtn1() {
    // Defensive check - ensure modal type is set and valid
    ...
    if (modalType === 'cycle') {
      ...
      this.saveSessionAndReset();
    }
  }

  handleGlobalModalBtn2() {
    // Defensive check - ensure modal type is set and valid
    if (!this.currentModalType) return;
  ...
  clearPendingModal() {
    ...
    this.closeGlobalModal();
    // Reset modal type after clearing
    this.currentModalType = null;
  }
 ...
 resetToDefaults() {
  ...
  playAlarm() {
    ...
    // If not stored, try to find it in DOM (for cases where this object was created before audio element)
    const audioElement = document.getElementById('alarmSound');
    if (audioElement) {
      this.alarmSound = audioElement;
      this.alarmSound.play().catch(err => console.error('Error playing alarm:', err));
    }
  }
