...
function handleUpload() {
  ...
  // Show status INSIDE flashcard FRONT
if (flashFront) flashFront.innerHTML = "📄 Processing document...";
cancelBtn.style.display = "inline-block";

setTimeout(() => {
  if (flashFront && flashFront.innerHTML.includes("Processing")) {
    flashFront.innerHTML = "✨ Generating flashcards...";
  }
}, 2000);
  ...
  fetch("/flashcards/upload", {
    ...
    .then((data) => {
      ...
      if (data.error) {
        if (flashFront) flashFront.innerHTML = `❌ Error: ${data.error}`;
        return;
      }
      flashcards = data.flashcards;
      currentIndex = 0;
      isFlipped = false;
      if (flashcards.length > 0) {
        showCard(currentIndex); // flashBack will now show card answers
      } else {
        if (flashBack) flashBack.innerHTML = "No flashcards found in document.";
      }
    })
    .catch((err) => {
      ...
      if (err.name === "AbortError") {
        if (flashFront) flashFront.innerHTML = "Upload canceled.";
      } else {
        if (flashFront) flashFront.innerHTML = `❌ Error: ${err.message}`;
      }
      cancelBtn.style.display = "none";
    });
}
